...
export function createGridNav(maze, { speed = 6, cellW = 10 } = {}) {
  ...
  function turn(dir) {
    ...
    // pressed side closed. ONLY when the runner is actually STOPPED at the wall (a deliberate
    // "I'm stuck at this corner, get me out" press) do we take the OTHER open side. We do NOT
    // auto-correct mid-approach, or a buffered EARLY press for a later junction could get
    // flipped to the opposite direction at an intermediate forced corner. (Never reverses —
    // turning back is the deliberate U-turn via Q / turnAround.)
    if (atWall) {
